...
fn main() {
    ...
    fn callback() { println!("this is a function!") }

    let menu_options = vec![
        MenuOption::new("run function", callback),
        MenuOption::new("run lambda", || println!("this is a lambda")),
    ];

    let mut menu = Menu::new(menu_options, MenuProps {
        title: "Some Different Options",
        ..MenuProps::default()
    });
    menu.show();
}
